...
const RoomDetail = () => {
  ...
  const { selectedRoom, loading, submitting, error } = useAppSelector(
    (s) => s.room,
  );
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <span
            className={`px-4 py-1.5 rounded-full text-sm font-semibold ${statusColors[selectedRoom.status]}`}
          >
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-2 gap-6">
              ...
              <InfoItem
                ...
                value={formatPeople(selectedRoom.maxPeople)}
              />
              <InfoItem
                label="Quản lý"
                value={formatText(selectedRoom.managerName)}
              />
              <InfoItem
                label="Ngày tạo"
                value={
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <PriceCard
                ...
                value={formatCurrency(selectedRoom.price)}
                accent
              />
              <PriceCard
                label="Giá điện"
                value={formatCurrency(selectedRoom.electricPrice, "₫/kWh")}
              />
              <PriceCard
                label="Giá nước"
                value={formatCurrency(selectedRoom.waterPrice, "₫/m³")}
              />
            </div>
          </div>
...
const PriceCard = ({
  ...
  <div
    className={`rounded-lg p-4 ${accent ? "bg-violet-50 border border-violet-200" : "bg-slate-50"}`}
  >
    <p className="text-sm text-slate-500 mb-1">{label}</p>
    <p
      className={`text-xl font-bold ${accent ? "text-violet-600" : "text-slate-900"}`}
    >
      {value}
    </p>
  </div>
