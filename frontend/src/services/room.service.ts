...
export const roomListService = async (page = 0, size = 2): Promise<RoomListResponse> => {
...
export const roomByBuildingService = async (buildingId: number, page = 0, size = 2): Promise<RoomListResponse> => {
  const response = await roomApi.getRoomsByBuilding(buildingId, page, size);
  if (!response.data) throw new Error(response.message || "Lấy danh sách phòng thất bại");
  return response.data;
};

export const roomByStatusService = async (status: RoomStatus, page = 0, size = 2): Promise<RoomListResponse> => {
  console.log("Fetching rooms with status:", status, "page:", page, "size:", size);
